...
const CONFIG = {
  trayHalf: 5.0,         // half-size of square boundary
  ...
  startEdgeMargin: 0.5,    // margin from wall for start positions
...
function simulateOnePath(startX, startZ, targetX, targetZ) {
  ...
  const wallLimit = CONFIG.trayHalf - CONFIG.wallCollisionMargin;
  ...
  for (let attempt = 0; attempt < CONFIG.trialsPerPath; attempt++) {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      // Wall bounce (square)
      if (Math.abs(x) > wallLimit) {
        x = Math.sign(x) * wallLimit;
        if (Math.sign(vx) === Math.sign(x)) {
          vx = -vx * CONFIG.wallBounce;
          bounceCount++;
        }
      }
      if (Math.abs(z) > wallLimit) {
        z = Math.sign(z) * wallLimit;
        if (Math.sign(vz) === Math.sign(z)) {
  ...
  for (let count = 1; count <= CONFIG.maxDice; count++) {
    ...
    for (let set = 0; set < CONFIG.setsPerCount; set++) {
      ...
      for (let d = 0; d < count; d++) {
        // Start near edge of square boundary
        const edgeLimit = CONFIG.trayHalf - CONFIG.startEdgeMargin;
        const side = (d + set * 3) % 4; // spread across 4 sides
        const along = (Math.random() - 0.5) * 2 * edgeLimit;
        let startX, startZ;
        if (side === 0)      { startX = edgeLimit;  startZ = along; }
        else if (side === 1) { startX = -edgeLimit; startZ = along; }
        else if (side === 2) { startX = along; startZ = edgeLimit; }
        else                 { startX = along; startZ = -edgeLimit; }
